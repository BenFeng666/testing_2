...
def load_checkpoint_model(base_model_path, checkpoint_path):
    ...
    # Force PEFT to skip ANY unknown keys
    model = PeftModel.from_pretrained(
        model,
        checkpoint_path,
        ignore_mismatched_params=True
    )
...
def predict_tox_eff(model, tokenizer, smiles):
    """
    Generate toxicity + predicted molecular score for a single SMILES.
    Uses chat_template, attention_mask, and deterministic decoding.
    """
...
"""
    ...
    messages = [
        {
            "role": "system",
            "content": (
                "You are a helpful assistant specialized in drug discovery and molecular analysis. You can predict molecular scores based on their SMILES structures Focus on predicting the molecular score."
            )
        },
        {"role": "user", "content": prompt}
    ]

    # Format into chat-style text
    ...
    # 🔥 Important: include attention_mask so InternLM2/ChemLLM can build KV cache
    inputs = tokenizer(
        [text],
        return_tensors="pt",
        padding=True,
...
def load_efficiency_dataset(efficiency_test_path, limit=200):
    ...
    with open(efficiency_test_path, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            data.append({
                "smiles": smiles,
                "true_toxicity": 0,   # no toxicity label in efficiency file
                "true_efficiency": eff
            })
    return data
...
def evaluate_checkpoint(
    ...
    for i, item in enumerate(eff_subset):
        ...
        print(
            f"[EFF] {i+1}/200 | SMILES={smiles} | "
            f"True_Tox={true_tox}, Pred_Tox={pred_tox} | "
            f"True_Eff={true_eff}, Pred_Eff={pred_eff}"
        )
    eff_true = np.array(eff_true_list)
    eff_pred = np.array(eff_pred_list)

    eff_within1_accuracy = float(np.mean(np.abs(eff_true - eff_pred) <= 1)) if len(eff_true) > 0 else 0.0
    eff_mae = float(mean_absolute_error(eff_true, eff_pred)) if len(eff_true) > 0 else 0.0
    eff_rmse = float(np.sqrt(mean_squared_error(eff_true, eff_pred))) if len(eff_true) > 0 else 0.0
    print(eff_within1_accuracy)
    # =======================================================
    ...
    # METRICS
    # =======================================================
    tox_true_np = np.array(tox_true_all)
    tox_pred_np = np.array([t if t is not None else -1 for t in tox_pred_all])
    valid_mask = tox_pred_np != -1

    tox_accuracy = np.mean(
        tox_true_np[valid_mask] == tox_pred_np[valid_mask]
    ...
    results = {
        ...
        "toxicity_samples": int(valid_mask.sum()),
    ...
    print("\n=== FINAL SUMMARY ===")
    print(f"Toxicity samples (valid preds): {int(valid_mask.sum())}")
