"""
Evaluate the finetuned Qwen3-30B model on test data
Accuracy metric: predictions within ±1 error are considered correct
"""
...
def evaluate_model(
    model_path="../checkpoints/qwen_30b_lora_finetuned",
    ...
    print(f"Loading Qwen2-32B model from: {model_path}")
    ...
    # Calculate accuracy - within ±1 error is considered correct
    correct_predictions = np.sum(np.abs(predictions - ground_truths) <= 1)
    accuracy = correct_predictions / total * 100 if total > 0 else 0
    exact_match_rate = exact_matches / total * 100 if total > 0 else 0
    
    print(f"\n{'='*60}")
    print(f"EVALUATION RESULTS - Qwen3-30B Model")
    print(f"{'='*60}")
    print(f"Total samples evaluated: {total}")
    print(f"Accuracy (±1 error allowed): {accuracy:.2f}%")
    ...
    results = {
        ...
        'accuracy': accuracy,  # Within ±1 considered correct
    ...
    results_file = f'{output_path}/evaluation_results_30b.json'
